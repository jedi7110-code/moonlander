...
import {box,cylinder,pipe} from '../../src/obs/materials.js';
...
export function sampleGrooming(seconds){
  ...
  return{time,motionTime:t,phase,label:labels[phase],hair:1-.5*(s(t,9,15)+s(t,21,27)),beard:1-s(t,33,43),
    x:VANITY.standX*enter,z:THREE.MathUtils.lerp(.78,VANITY.z,enter)*(1-leave)+.78*leave,
    yaw:t<46?Math.PI+Math.PI/2*s(t,4,5.5):Math.PI*1.5+Math.PI/2*s(t,46,48),
    moving:(t>0&&t<4)||(t>48&&t<52),walkDistance:(.78-VANITY.z)*(enter+leave),done:t===52};
}
...
export function createVanity(m,metals){
  ...
  const shape=new THREE.Shape();shape.moveTo(-.46,-.27);shape.lineTo(.46,-.27);shape.lineTo(.46,.27);shape.lineTo(-.46,.27);shape.closePath();
...
export function prepareGroomingMotion(milo,station,tools){
  ...
  return{update(seconds){
    ...
    animateMilo(milo,{moving:state.moving,time:0,walkDistance:state.walkDistance,action:null,dt:0});
    ...
    milo.position.set(state.x,VANITY.floor,state.z);milo.rotation.y=state.yaw;
    ...
    head.rotation.set(-.10*chin,.16*(rightCheek-leftCheek),.065*(leftCut-rightCut));body.position.y=0;milo.updateWorldMatrix(true,true);
