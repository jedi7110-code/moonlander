...
2026-09-14の座りシルエット調整: 最新の実猫写真に合わせ、顎を膨らませる補正は廃止し、`SitChest` で前脚の付け根の上にある胸を前方へ張り出させています。座り姿勢の座標で胸を選び、首の付け根から前脚へなだらかに変位を弱め、顎・顔は変更しません。座り姿勢で作った変位を元のメッシュ座標へ戻し、座る・毛づくろい・座って遊ぶときだけ滑らかに適用。立ち姿・歩行・食事・睡眠の表面は変更しません。座り系の後ろ足の接地点を前へ0.026モデル単位移した調整は維持します。床への接地、胸の変化量、顎の不変性、待機・歩行データの維持を `tests/obs-lucy.test.js` で確認します。

追加の実猫写真に合わせ、`SitBelly` で座ったときの腹部にも前方・左右の厚みを追加。元の連続メッシュの腹側を広げ、隣接頂点間で補正量をならして局所的な突起を抑えます。胸・顎・足先の形状はこの補正から除外し、座り系の動作だけに適用します。

続く調整では首下から胸への厚みを約15%増し、`SitPaws` で後ろ足の甲・かかと寄りの上面を低くして、足先を少し前へ広げています。前脚・顎・歩行時の形状は維持し、座り系の足裏が床へめり込まないことを確認します。
