...
test('feet remain on the pedals and hands on the handlebar throughout a full cycle',()=>{
  ...
  const fresh=createMilo(m);animateMilo(fresh,{action:null,moving:true,facing:1,time:3});
  assert.deepEqual(milo.userData.chest.quaternion.toArray(),fresh.userData.chest.quaternion.toArray());
  assert.deepEqual(milo.userData.head.position.toArray(),fresh.userData.head.position.toArray());
  for(const {arm,hand,side}of milo.userData.arms){
    assert.deepEqual(arm.position.toArray(),fresh.userData.arms.find(a=>a.side===side).arm.position.toArray());
    assert.equal(hand.rotation.x,0);
  }
