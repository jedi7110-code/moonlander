...
export const BUNK_PHASE_SECONDS={approaching:1,opening:2.4,extending:1.4,sitting:1.8,settled:.35,lowering:BED_ENTRY_SECONDS,entering:1.4,closing:2.7,waking:2.7,leaving:1.4,rising:BED_ENTRY_SECONDS,seated:.35,standing:1.8,retracting:1.4,sealing:2.4,departing:1};
...
export class BunkVisit {
  constructor({entered=()=>{},exited=()=>{},startAsleep=false,sequential=true}={}){
    ...
    this.seconds=sequential?BUNK_PHASE_SECONDS:{...BUNK_PHASE_SECONDS,lowering:3,rising:3};
  }
  requestExit(){
  ...
  update(dt){
    ...
    while(remaining>0&&this.phase!=='sleeping'&&this.phase!=='done'){
      const duration=this.seconds[this.phase],step=Math.min(remaining,duration-this.age);
  ...
  get pose(){
    const phase=this.phase,u=Math.max(0,Math.min(1,this.age/(this.seconds[phase]??1))),t=smooth(u);
    ...
    const entry=phase==='lowering'?u:phase==='rising'?1-u:recline;
    const motion=bedEntryMotion(entry),turn=this.sequential?motion.turn:smooth((recline-.24)/.48),trayDepth=BUNK_BED.depth+BUNK_TRAY.travel*tray;
    const standing=BUNK_TRAY.walkDepth+(BUNK_TRAY.standingDepth-BUNK_TRAY.walkDepth)*approach;
    const shift=this.sequential?motion.depth:turn;
    const depth=(standing+(BUNK_TRAY.seatDepth-standing)*seat)*(1-shift)+trayDepth*shift;
    ...
    return {phase,age:this.age,approach,seat,recline,turn,tray,depth,open,lift,...(this.sequential?{entry,bedDepth:trayDepth}:{}),occupied:seat>0,light:phase==='sleeping'?.05:phase==='closing'?1-.95*t:phase==='waking'?.05+.95*t:1};
  }
}
