...
import {animateMedical,medicalRecline,MED_BED} from './medical.js';
...
export class ObservationView {
  ...
  render(dt,time,actor,brain,catRoutine,care,paused=false,airlock=null){
    ...
    animateMilo(this.milo,{moving:actor.busy,waiting:actor.waitingForHatch,climbing:actor.climbing,facing:actor.facing,action,time,actionTime,knock:actor.knockTime});
    animateMedical(this.ship.medical,actionTime,action==='medical',action==='medical'?brain.medicalSample:brain.lastMedicalReport);
    ...
    animateCat(this.cat,{time,moving:catMoving,climbing:false,facing:catMotion.facing,mode:catRoutine.mode,passage});
    ...
    if(action==='medical')this.milo.position.z=THREE.MathUtils.lerp(.78,MED_BED.depth,medicalRecline(actionTime));
