...
export const HATCH_TRAVEL=1.90;
...
export function cargoPose(age,index){
  ...
  if(time<0)return{visible:false,x:-(index-1)*.20,y:.75,z:-2.5,tilt:0};
  if(time<fall){const t=time/fall;return{visible:true,x:-(index-1)*.20*(1-smooth((t-.6)/.4)),y:.75*(1-t*t),z:-2.5+2.5*smooth(t),tilt:(index%2?1:-1)*.09*Math.sin(t*Math.PI)};}
